#!/usr/bin/env python3
"""Run the equal-size STRICT_LARGE vs MILD_LARGE five-seed diagnostic."""
...
MILD_LARGE_SHA256 = "E7638533CC73B91E5F9BFB065344B7993D9E666E1CD3A4D5B97DEF48863AC4C8"


def load_base_runner() -> Any:
    spec = importlib.util.spec_from_file_location(
        "large_strict_mild_base_runner",
        BASE_RUNNER_PATH,
    )
...
def sample_metadata(path: Path) -> dict[str, tuple[str, int, str]]:
    metadata: dict[str, tuple[str, int, str]] = {}
    with path.open(encoding="utf-8-sig", newline="") as handle:
        reader = csv.DictReader(handle)
        required = {"sample_id", "split", "model_label", "source_policy"}
        ...
        for row in reader:
            if row["split"] not in ("train", "validation"):
                continue
            value = (row["split"], int(row["model_label"]), row["source_policy"])
            previous = metadata.setdefault(row["sample_id"], value)
...
def prepare_data(
    ...
    assert len(data["STRICT_TRAIN"][0]) == 1500
    assert len(data["MILD_TRAIN"][0]) == 1500
    assert len(data["STRICT_VAL"][0]) == 150
    assert len(data["MILD_VAL"][0]) == 150
    base.assert_class_counts(data["STRICT_TRAIN"][2], 300, "STRICT_LARGE Train")
    base.assert_class_counts(data["MILD_TRAIN"][2], 300, "MILD_LARGE Train")
    base.assert_class_counts(data["STRICT_VAL"][2], 30, "STRICT_LARGE Validation")
    base.assert_class_counts(data["MILD_VAL"][2], 30, "MILD_LARGE Validation")

    strict_train_ids = set(data["STRICT_TRAIN"][0])
    mild_train_ids = set(data["MILD_TRAIN"][0])
    ...
    assert len(strict_train_ids & mild_train_ids) == 1355
    assert len(strict_train_ids - mild_train_ids) == 145
    assert len(mild_train_ids - strict_train_ids) == 145
    print("VALIDATION_IDENTICAL=True")
    print("STRICT_LARGE_TRAIN_COUNTS=300,300,300,300,300")
    print("MILD_LARGE_TRAIN_COUNTS=300,300,300,300,300")
    print("VALIDATION_COUNTS=30,30,30,30,30")
    print("COMMON_STRICT_COUNTS=271,271,271,271,271")
    print("STRICT_ONLY_COUNTS=29,29,29,29,29")
    print("MILD_ONLY_COUNTS=29,29,29,29,29")
...
def print_seed_result(seed: int, arm: str, result: dict[str, Any], classes: tuple[str, ...]) -> None:
    ...
    for index, class_name in enumerate(classes):
        print(f"{class_name},{result['recall'][index]:.6f},{result['f1'][index]:.6f}")
...
def main() -> None:
    ...
    print("GPU_NAME=" + (torch.cuda.get_device_name(0) if torch.cuda.is_available() else "N/A"))
    ...
    print(
        ...
        f"num_classes={base.NUM_CLASSES},d_model={base.D_MODEL},nhead={base.NHEAD},"
        f"num_layers={base.NUM_LAYERS},kv_heads={base.KV_HEADS},dropout={base.DROPOUT},"
    ...
    strict_path = args.strict_large_csv.resolve()
    mild_path = args.mild_large_csv.resolve()
    strict_hash = base.sha256_file(strict_path)
    mild_hash = base.sha256_file(mild_path)
    assert strict_hash == STRICT_LARGE_SHA256
    assert mild_hash == MILD_LARGE_SHA256
    print(f"STRICT_LARGE_SHA256={strict_hash}")
    print(f"MILD_LARGE_SHA256={mild_hash}")

    strict_metadata = sample_metadata(strict_path)
    mild_metadata = sample_metadata(mild_path)
    assert Counter(value[2] for value in strict_metadata.values()) == Counter({"strict_1s": 1650})
    assert Counter(value[2] for value in mild_metadata.values()) == Counter(
        {"strict_1s": 1505, "mild_2s_le5_span205": 145}
    )
    ...
    for seed in SEEDS:
        ...
        print_seed_result(seed, "STRICT_LARGE", strict_result, base.CLASSES)
        print_seed_result(seed, "MILD_LARGE", mild_result, base.CLASSES)
        print(f"=== SEED {seed} MILD - STRICT ===")
        print(f"accuracy_delta={mild_result['accuracy'] - strict_result['accuracy']:.6f}")
        print(f"macro_f1_delta={mild_result['macro_f1'] - strict_result['macro_f1']:.6f}")
